...
<body class="bg-zinc-50 min-h-screen w-full flex items-center justify-center p-4 relative overflow-x-hidden" oncontextmenu="return false;">
    ...
    <div class="w-full max-w-md bg-white/80 backdrop-blur-xl p-8 rounded-[2rem] shadow-2xl border border-white/60 relative z-10 my-4">
        ...
        <div class="text-center mb-6">
            ...
            <h1 class="text-2xl font-bold text-zinc-900 tracking-tight">Daftar PARI ID</h1>
            <p class="text-zinc-500 text-sm mt-1 font-medium">Bergabung untuk pengalaman belanja terbaik</p>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            <!-- Nama -->
            <div>
                <label class="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5 ml-1">Nama Lengkap</label>
            ...
            <!-- WhatsApp -->
            <div>
                <label class="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5 ml-1">Nomor WhatsApp</label>
                <div class="relative group">
                    <span class="absolute left-4 top-3.5 text-zinc-400 group-focus-within:text-green-600 transition-colors">
                        <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <input type="text" name="whatsapp" required placeholder="08xx" value="{{ old('whatsapp') }}" 
                           class="w-full pl-12 pr-4 py-3.5 bg-zinc-50/50 border border-zinc-200 rounded-xl text-sm font-bold text-zinc-900 focus:ring-2 focus:ring-green-500 focus:border-transparent focus:bg-white outline-none transition-all">
                </div>
                @error('whatsapp') 
                    <small class="text-red-500 text-xs mt-1 ml-1">{{ $message }}</small> 
            ...
            <!-- Nama Tele (Username Telegram) -->
            <div>
                <label class="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5 ml-1">
                    Username Telegram <span class="text-blue-500">(tanpa @)</span>
                </label>
                <div class="relative group">
                    <span class="absolute left-4 top-3.5 text-zinc-400 group-focus-within:text-blue-600 transition-colors">
                        <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
                        </svg>
                    </span>
                    <input type="text" name="nama_tele" required placeholder="username" value="{{ old('nama_tele') }}" 
                           class="w-full pl-12 pr-4 py-3.5 bg-zinc-50/50 border border-zinc-200 rounded-xl text-sm font-bold text-zinc-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent focus:bg-white outline-none transition-all">
                </div>
                <small class="text-xs text-zinc-400 mt-1 ml-1 block">
                    Email akan otomatis dibuat: <span id="email-preview" class="font-bold text-green-600"></span>
                </small>
                @error('nama_tele') 
                    <small class="text-red-500 text-xs mt-1 ml-1">{{ $message }}</small> 
                @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="relative group">
                        ...
                        <button type="button" onclick="togglePass('pass1', this)" class="absolute right-3 top-3.5 text-zinc-400 hover:text-zinc-600 focus:outline-none">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div>
                    <label class="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5 ml-1">Konfirmasi Password</label>
                    <div class="relative group">
                        <span class="absolute left-4 top-3.5 text-zinc-400 group-focus-within:text-green-600 transition-colors">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <input type="password" name="password_confirmation" id="pass2" required placeholder="••••••" 
                               class="w-full pl-12 pr-10 py-3.5 bg-zinc-50/50 border border-zinc-200 rounded-xl text-sm font-bold text-zinc-900 focus:ring-2 focus:ring-green-500 focus:border-transparent focus:bg-white outline-none transition-all">
                        <button type="button" onclick="togglePass('pass2', this)" class="absolute right-3 top-3.5 text-zinc-400 hover:text-zinc-600 focus:outline-none">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                            </svg>
        ...
        <div class="mt-8 text-center">
            <p class="text-xs text-zinc-500 font-medium">
                Sudah punya akun? <a href="{{ route('login') }}" class="font-bold text-green-600 hover:underline">Masuk disini</a>
            </p>
        </div>
    </div>

    <script>
        // Preview email otomatis
